...
class Wallet():
    ...
    def getTransactions(self, address):
        ...
        # Add the 'type' field to transactions and indicate whether it's 'sent' or 'received'
        for txn in asSender:
            txn['type'] = 'Sent'

        for txn in asReceiver:
            txn['type'] = 'Received'
        
        # Combine the transactions
        allUserTnx = asSender + asReceiver
        print(allUserTnx,"before")
        # Sort the combined transactions by the 'time' field
        allUserTnx = sorted(allUserTnx, key=lambda txn: txn.get('time', 0))  # Replace 'time' with your actual timestamp field name
        print(allUserTnx,"this")
        return [allUserTnx, []]
